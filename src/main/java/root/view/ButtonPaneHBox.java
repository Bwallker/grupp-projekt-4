...
public class ButtonPaneHBox extends HBox {
	...
	public ButtonPaneHBox(MusicOrganizerController contr, MusicOrganizerWindow view) {
		...
		this.flagButton = createFlagButton();
		this.getChildren().add(flagButton);

		this.gradeButton = createGradeButton();
	...
	private Button createUndoButton() {
		...
		button.setTooltip(new Tooltip("Undoes the last performed action"));
		button.setMinWidth(BUTTON_MIN_WIDTH);
	...
	private Button createRedoButton() {
		...
		button.setTooltip(new Tooltip("Redoes the last undone action"));
		button.setMinWidth(BUTTON_MIN_WIDTH);
		button.setOnAction(arg0 ->
				controller.redo()
		);
	...
	private Button createFlagButton() {
		...
		button.setTooltip(new Tooltip("Toggle the flag on all selected sound clips"));
		button.setMinWidth(BUTTON_MIN_WIDTH);
		button.setOnAction(arg0 ->
				controller.flag()
		);
	...
	private Button createGradeButton() {
		...
		button.setTooltip(new Tooltip("Grade all selected sound clips"));
		button.setMinWidth(BUTTON_MIN_WIDTH);
		button.setOnAction(arg0 ->
				controller.grade()
		);
